...
#               
#               if (middleAntennaStartIndex + 1 <= upstreamAntennas_first &&
#                   grepl(paste0("^(", paste(antenna_i, collapse = "|"), ")"), tag_data$Event[middleAntennaStartIndex + 1])) {
#                 
...
#                 } else {
#                   print(paste("Fish hit other antennas than", paste(antenna_i, collapse = ", "), "after", downstreamAntennas, "so sequence doesn't work"))
#                   middle_success <- FALSE
#                   break
#                 }
#               } else {
#                 print(paste("Fish hit", tag_data$Event[middleAntennaStartIndex + 1], "instead of", paste(antenna_i, collapse = ", "), "after", downstreamAntennas, "so sequence doesn't work"))
#                 middle_success <- FALSE
#                 break
#               }
#             }
#             
#             if (middle_success && grepl(paste0("^(", paste(middleAntennas[[length(middleAntennas)]], collapse = "|"), ")"), tag_data$Event[upstreamAntennas_first - 1])) {
...
extract_sequences <- function(tag_data, downstreamAntennas, middleAntennas, upstreamAntennas) {
  ...
  while (i < nrow(tag_data)) {
    ...
    # Find indices of events that match the chosen downstream antennas within the subset of tag_data starting from the current index i
    downstreamAntennas_index <- which(grepl(paste0("^(", paste(downstreamAntennas, collapse = "|"), ")"), tag_data$Event[i:nrow(tag_data)]))
    adjusted_downstream = downstreamAntennas_index + i -1
    ...
    if (length(upstreamAntennas_index) > 0 & length(downstreamAntennas_index) > 0) {
      ...
      # Get the absolute position in tag_data of the first upstream antenna event found
      #this is where index gets adjusted; absolute position
      upstreamAntennas_first <- i + upstreamAntennas_index[1] - 1
