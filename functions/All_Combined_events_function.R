...
All_combined_events_function <- function(Stationary, Mobile, Biomark, Release, Recaptures, allPressureTransducerDataWithDischarge, windyGapContinuous){
  ...
  condensedWGFP <- Stationary %>%
    select(DTY_DSTcorrected, ARR_DSTcorrected, TAG, SCD, UTM_X, UTM_Y) %>%
    rename(Scan_Date = DTY_DSTcorrected, Scan_Time = ARR_DSTcorrected, Site_Code = SCD, UTM_X = UTM_X, UTM_Y = UTM_Y)
