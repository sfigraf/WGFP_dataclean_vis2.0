...
wgfpMetadata <- lapply(excel_sheets("./data/WGFP Metadata.xlsx"), function(x)
  if(x == "MarkerTagIssues"){
    read_excel("./data/WGFP Metadata.xlsx", sheet = x)
  } else{
    read_excel("./data/WGFP Metadata.xlsx", sheet = x, col_types = "text")
  }
)
names(wgfpMetadata) <- excel_sheets("./data/WGFP Metadata.xlsx")
# wgfpMetadata$MarkerTagIssues$IssueStartDatetime <- as.POSIXct(wgfpMetadata$MarkerTagIssues$IssueStartDatetime, format = "%m/%d/%Y %I:%M:%S %p")
x <- wgfpMetadata$MarkerTagIssues
