...
Stationary <- readRDS("./data/WGFP_Raw_20240403.rds")
...
```{r USGSData, include=FALSE, echo = FALSE}
usgsTime <- Sys.time()
...
windyGap <- readNWISuv(siteNumbers = "09034250", #code for windy gap
                       parameterCd = "00060", #this is parameter code for discharge; more can be added if needed
...
usgsTime2 <- Sys.time()
print(paste("Reading in USGS Discharge Data took", round((usgsTime2-usgsTime),2)))
windyGap <- renameNWISColumns(windyGap)

allPressureTransducerDataWithDischarge <- windyGap[,c("dateTime", "Flow_Inst")] %>%
  #we're going to get a "many" relatoinship here because the discharge is joined to each site reading so it causes each disharge timestamp that has a matching PT logger reading to get multiplied by at least 3 (1 for RB, HP, and CF)
  left_join(allPressureTransducerData, by = c("dateTime"= "DateTime")) %>%
  rename(USGSDischarge = Flow_Inst) %>%
  #we are gertting rid of the "ice" entries here by turning them to 0
