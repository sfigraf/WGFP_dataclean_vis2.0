...
allPressureTransducerData <- windyGap[,c("dateTime", "Flow_Inst")] %>%
  left_join(allPressureTransducerData, by = c("dateTime"= "DateTime")) %>%
  rename(USGS_Discharge_at_HitchingPost = Flow_Inst)

# allPressureTransducerData <- allPressureTransducerData %>%
#   left_join(windyGap[,c("dateTime", "Flow_Inst")], by = c("DateTime" = "dateTime")) %>%
#   rename(USGS_Discharge_at_HitchingPost = Flow_Inst)
```
...
allFlatFiles <- list(
  ...
  "PTData" = allPressureTransducerData, 
  "USGSDischargeData" = windyGap
)
