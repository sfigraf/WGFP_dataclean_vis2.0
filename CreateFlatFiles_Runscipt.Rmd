...
##some stationary hosuekeeping

Stationary_cleanSiteCodes <- Stationary %>%
  filter(DTY >= as.Date("2020-08-06")) %>%
  mutate(
...
Stationary_cleanMarkerTags <- Stationary_cleanSiteCodes %>%
  filter(str_detect(TAG, "^0000000"))
Stationary_Marker_tags <- Stationary_cleanMarkerTags %>%
  #this is to get it in right format for displaying
  mutate(DTY = ymd(DTY))

Cleaned_Stationary_FishdetectionsOnly <- Stationary_cleanSiteCodes %>%
  filter(grepl("^900", TAG),
             Code %in% c("I", "S")) %>% 
  mutate(TAG = ifelse(grepl("^900", TAG), gsub("^900", "", TAG)))
...
dummy_rows_list <- add_dummy_rows(stationary = Stationary$Cleaned_Stationary_detectionsOnly, biomark = Biomark, release1 = Release)
...
#Stationary <- Stationary$Cleaned_Stationary_detectionsOnly
...
allFlatFiles <- list(
  ...
  "Stationary_Marker_tags" = Stationary_Marker_tags
...
saveRDS(allFlatFiles, "data/allFlatFiles.rds")
