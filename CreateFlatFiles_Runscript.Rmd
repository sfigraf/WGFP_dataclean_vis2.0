...
source(paste0("./miscR/map_polygon_readins.R"))
...
Stationary <- readRDS("./data/WGFP_Stationary_Raw_20260527.rds")

Mobile <- read.csv("./data/WGFP_Mobile_Detect_AllData.csv" , colClasses= c(rep("character", 14), rep("numeric", 4), rep("character", 3)))

Biomark <- readRDS("./data/WGFP_Biomark_Raw_20260527.rds")

# need to have tagID as a numeric field in the .csv file in order to be read in correctly as opposed to 2.3E+11 

Release <- read.csv("./data/WGFP_ReleaseData_Master_20260430.csv", na.strings = c(""," ","NA"), colClasses=c(rep("character",7), "numeric", "numeric",rep("character",9))) %>%
  janitor::remove_empty()
Recaptures <- read.csv("./data/WGFP_RecaptureData_Master_20260430.csv", na.strings = c(""," ","NA"), colClasses = c(rep("character", 7), rep("numeric", 2), rep("character", 8))) %>%
  janitor::remove_empty()
...
wgfpMetadata <- lapply(excel_sheets("./data/WGFP Metadata.xlsx"), function(x)
  if(x == "MarkerTagIssues"){
    read_excel("./data/WGFP Metadata.xlsx", sheet = x, col_types = c("guess", "date", "date", "guess", "guess", "guess", "guess"))
  } else if(x == "DeploymentDurations") {
        read_excel("./data/WGFP Metadata.xlsx", sheet = x, col_types = c("guess", "guess", "guess", "date", "date"))
...
windyGapDaily <- read_waterdata_daily(monitoring_location_id = "USGS-09034250", #code for windy gap
                       ...
                       time = c("2020-08-06", Sys.Date())
)
windyGapDaily <- windyGapDaily %>%
  st_drop_geometry() %>%
...
windyGap <- readNWISuv(siteNumbers = "09034250", #code for windy gap
                       parameterCd = c("00060", "00010", "00065"), #this is parameter code for discharge; more can be added if needed
...
# 1. Set up your total date range
#needs to be denver
start_date <- as.POSIXct("2020-08-06 00:00:00", tz = "America/Denver")
end_date <- Sys.time()

# 2. Create the sequence of start dates (every 2 years)
starts <- seq(from = start_date, to = end_date, by = "2 years")

# 3. Create the sequence of end dates
# We take all the start dates (skipping the first one), subtract 1 day, 
# and then tack your final end_date onto the very end.
ends <- c(starts[-1] - 1, end_date)

# 3. Loop through the intervals, pull the data, and bind the rows together
...
windyGap2 <- map2_dfr(
  ...
  ~ read_waterdata_continuous(
      monitoring_location_id = "USGS-09034250",
      parameter_code = c("00060", "00010", "00065"),
      time = c(as.character(.x), as.character(.y))
  )
)

windyGap2_wide <- windyGap2 %>%
  #mutate(time = lubridate::with_tz(time, tzone = "America/Denver")) %>%
  # 1. Drop the columns we no longer want
  #select(-statistic_id, -unit_of_measure) %>%
  
  # 2. Translate the numeric codes into human-readable names
  mutate(
    parameter_code = replace_values(
      ...
      #.default = parameter_code # Keeps any unexpected codes as their original numbers
    )
  ) %>%
...
windyGap2Renamed <- windyGap2_wide %>%
  mutate(USGSWatertemp = (`USGSWatertemp` * 9/5) + 32) %>%
  rename(dateTime = time)


# windyGap2 <- read_waterdata_continuous(
#   monitoring_location_id = "USGS-09034250",               # Agency code prefix is now required
#   parameter_code = c("00060", "00010", "00065"),          # Argument name changed
#   time = c("2025-08-06", as.character(Sys.Date()))        # Start and end date combined into a 'time' vector
# )

windyGapRenamed <- renameNWISColumns(windyGap) %>%
  mutate(Wtemp_Inst = (Wtemp_Inst * 9/5) + 32) %>%
  rename(USGSDischarge = Flow_Inst, 
         USGSWatertemp = Wtemp_Inst, 
         USGSGageHeightFt = GH_Inst)
#tz
windyGap2RenamedQAQC <- windyGap2Renamed %>%
  select(dateTime, USGSDischarge, USGSGageHeightFt, USGSWatertemp)

windyGapRenamedQAQC <- windyGapRenamed %>%
    select(dateTime, USGSDischarge, USGSGageHeightFt, USGSWatertemp)

  #alignColumns(windyGap2Renamed, names(windyGapRenamed), windyGapRenamed)

x <- anti_join(windyGapRenamedQAQC, windyGap2RenamedQAQC)

joined <- windyGapRenamedQAQC %>%
  left_join(windyGap2RenamedQAQC, by = "dateTime") %>%
  mutate(dischrgeEqual = USGSDischarge.x == USGSDischarge.y, 
         tempEqual = USGSWatertemp.x == USGSWatertemp.y, 
        gageEqual = USGSGageHeightFt.x == USGSGageHeightFt.y, 

         
         )
tz(windyGapRenamedQAQC$dateTime)

tz(windyGap2RenamedQAQC$dateTime)



#this is to make attaching these readings to detections later
windyGapRenamed$dateTime <- lubridate::force_tz(windyGapRenamed$dateTime, tzone = "UTC") 


#for joining with WindyGapDaily since windy gap daily doesn't get daily average staff gage readings (code 00065)
avgGageHeightDaily <- windyGapRenamed %>%
  group_by(Date = date(dateTime)) %>%
  summarize(avgDailyGageHeight = round(mean(USGSGageHeightFt, na.rm = T), 2)) %>%
  #gets rid of NaNs; not really sure it's an issue but ensures unfiromity on data. mostly worreid about how it might get plotted
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .)))

windyGapDaily <- windyGapDaily %>%
  left_join(avgGageHeightDaily, by = "Date")

allPressureTransducerDataWithDischarge <- windyGapRenamed[,c("dateTime", "USGSDischarge", "USGSWatertemp", "USGSGageHeightFt")] %>%
...
allPressureTransducerDataWithDischarge_Long <- allPressureTransducerDataWithDischarge %>%
  pivot_longer(cols = c(contains("_"), "USGSDischarge", "USGSWatertemp", "USGSGageHeightFt"), names_to = "EnvVariable", values_to = "Reading")
...
Stationary_withUTMSDSTCorrected <- addUTMsAndCorrectDSTStationary(Stationary = Stationary, test_tags = test_tags)
...
AllCombinedEvents <- All_combined_events_function(Stationary = Stationary_withUTMSDSTCorrected, 
                                                  Mobile = Mobile, Release = Release, Biomark = Biomark, Recaptures = Recaptures, 
                                                  allPressureTransducerDataWithDischarge = allPressureTransducerDataWithDischarge, 
                                                  windyGapContinuous = windyGapRenamed)

Cleaned_Stationary_FishdetectionsOnly <- Stationary_withUTMSDSTCorrected %>%
...
combined_events_stations <- PrepareforStatesMovementsandSummary(DailyDetectionsStationsStates$spatialList$stationStateData)  

# states

# aplies enc_hist_summary wide function
enc_hist_wide_list <- Ind_tag_enc_hist_wide_summary_function(AllCombinedEvents$df_list$Recaps_detections, cleanedRelease, combined_events_stations, States_summarized = encounterMARKStates$States_summarized)
...
avianPredationList <- avianPredationListFunction(All_Events = AllCombinedEvents$df_list$All_Events, 
                                       AvianPredation = AvianPredation, 
                                       enc_hist_wide_listPossibleAvianPredation = enc_hist_wide_list$enc_wide_list$predationList, #predationList
...
if (nrow(AllCombinedEvents$df_list$QAQCtables$tagsWithMoreThan1Release) > 0) {
  cat('<span style="color:red; font-size: 30px;"><strong>The following tags have multiple "Release" Events</strong></span><br><br>')
  DT::datatable(AllCombinedEvents$df_list$QAQCtables$tagsWithMoreThan1Release)
}

if (nrow(AllCombinedEvents$df_list$QAQCtables$detectionsBeforeReleaseEvent) > 0) {
  cat('<span style="color:red; font-size: 30px;"><strong>Detections Before Official Release Event</strong></span><br><br>')
  DT::datatable(AllCombinedEvents$df_list$QAQCtables$detectionsBeforeReleaseEvent)
}

if (nrow(AllCombinedEvents$df_list$QAQCtables$tagsWithMoreThan1Species) > 0) {
  cat('<span style="color:red; font-size: 30px;"><strong>Tags with >1 Species Assigned</strong></span><br><br>')
  DT::datatable(AllCombinedEvents$df_list$QAQCtables$tagsWithMoreThan1Species)
}

if (nrow(DailyDetectionsStationsStates$spatialList$noUTMS) > 0) {
  cat('<span style="color:red; font-size: 30px;"><strong>The following tags could not be assigned stations because of an NA entry for UTMs:</strong></span><br><br>')
  DT::datatable(DailyDetectionsStationsStates$spatialList$noUTMS)
}

if (nrow(DailyDetectionsStationsStates$spatialList$noState) > 0) {
  cat('<span style="color:red; font-size: 30px;"><strong>The following tags could not be assigned a state (aside from TGM) because their boundary was outside the states polygon: </strong></span><br><br>')
  DT::datatable(DailyDetectionsStationsStates$spatialList$noState)
}
