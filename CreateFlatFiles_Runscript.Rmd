...
Stationary <- readRDS("./data/WGFP_Stationary_Raw_20251104.rds")
...
Release <- read.csv("./data/WGFP_ReleaseData_Master_20251031.csv", na.strings = c(""," ","NA"), colClasses=c(rep("character",7), "numeric", "numeric",rep("character",9))) %>%
...
windyGapDailyNewFunction <- read_waterdata_daily(monitoring_location_id = "USGS-09034250", #code for windy gap
...
windyGapDaily_new <- windyGapDailyNewFunction %>%
  st_drop_geometry() %>%
  # group_by(time) %>%
  # summarise(
  #   units = paste(unit_of_measure, collapse = ", "),
...
# x3 <- windyGapDaily_oldfunction %>%
#   filter(is.na(Wtemp))

windyGapDaily_newF <- windyGapDaily_new %>%
...
Movements_list <- get_movements_function(combined_events_stations, dailyUSGSData = windyGapDaily_newF, eventsWithPeriodsSelect = encounterMARKStates$eventsWithPeriodsSelectForMovementJoining, wgfpMetadata$TimePeriods)
