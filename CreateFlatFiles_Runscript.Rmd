...
Stationary <- readRDS("./data/WGFP_Raw_20241003.rds")

Mobile <- read.csv("./data/WGFP_Mobile_Detect_AllData.csv" , colClasses= c(rep("character", 14), rep("numeric", 4), rep("character", 3)))

Biomark <- readRDS("./data/Biomark_Raw_20241001.rds")

# need to have tagID as a numeric field in the .csv file in order to be read in correctly as opposed to 2.3E+11 

Release <- read.csv("./data/WGFP_ReleaseData_Master_20241009.csv", na.strings = c(""," ","NA"), colClasses=c(rep("character",8), "numeric", "numeric",rep("character",9)))
Recaptures <- read.csv("./data/WGFP_RecaptureData_Master_20241009.csv", na.strings = c(""," ","NA"), colClasses = c(rep("character", 8), rep("numeric", 2), rep("character", 8)))
...
DailyMovements_withStations <- spatial_join_stations_detections(condensedEvents = AllCombinedEvents$df_list$All_Events_most_relevant, simpleStations = simpleStations)

if(nrow(DailyMovements_withStations$spatialList$noUTMS) > 0){
  problemNAtagsMessage <- paste0("The following tags could not be assigned stations because of an NA entry for UTMs: ", unique(DailyMovements_withStations$spatialList$noUTMS$TAG),
                                         ".")
} else{
  problemNAtagsMessage <- ""
}

#prepares joined stations and relevant Movements dataset for movements summaries and states

combined_events_stations <- PrepareforStatesMovementsandSummary(DailyMovements_withStations$spatialList$stationData)

# states

states <- states_function(combined_events_stations, GhostTags, AvianPredation)
# aplies enc_hist_summary wide function
enc_hist_wide_list <- Ind_tag_enc_hist_wide_summary_function(AllCombinedEvents$df_list$Recaps_detections, cleanedRelease, combined_events_stations, states$states_df_list$States_summarized, markerTags = unique(Cleaned_Marker_tags$TAG))
...
WeeklyMovementsbyType <- WrangleMinicharts_function(Movements_df$dailyMovementsTable1)
...
### next: want to filter out tags already in the avian predation file
#
#incoporate into new QAQC tab on app 
avianPredationList <- list(
  ...
  "statesWeeklyActiveFish" = states$states_df_list$possibleAvianPredation$weeklyActiveFish, 
  "statesAllActiveFish" = states$states_df_list$possibleAvianPredation$overAllActiveFishNotinWeeklyDF 
  )

###making df of frewuency counts for each time a fish appears on a df to get "priority" list
tagCounts <- purrr::map_dfr(avianPredationList, ~ {
...
tagCountsNoPredation <- anti_join(tagCounts, AvianPredation, by = c("TAG" = "TagID"))
avianPredationList[["tagCountsNoPredation"]] = tagCountsNoPredation

###QAQC ghost tags
ghostTagsWithMovementAfterGhostDate <- qaqcGhostTagMovements(GhostTags, Movements_df$dailyMovementsTable1)

#this is used in states_data reactives
weeks <- data.frame(weeks_since = min(states$states_df_list$All_States$weeks_since):max(states$states_df_list$All_States$weeks_since))
...
movements_list <- list(
  "Movements_df" = Movements_df$dailyMovementsTable1,
...
#### `r DailyMovements_withStations$endMessage`
#### `r states$endMessage`
#### `r enc_hist_wide_list$endMessage`
#### `r Movements_df$message`
...
allFlatFiles <- list(
  ...
  "states_data_list" = states$states_df_list,
