...
library(gganimate)
library(basemaps)
library(gifski) #dependency hopfeully for
library(leaflet)
...
#because of the location of this doc, it's easier to bring in specific fiels from misc.R instead of sourcing the whole document
...
simpleStations <- readr::read_rds(file.path(layerLocation, "simpleStations.rds"))
...
Throughout the study, there have been a total of `r nrow(indiv_datasets_list$releasedata)` fish tagged with PIT tags. Of these fish, `r nrow(filter(indiv_datasets_list$releasedata, !Species %in% c("TGM", "MERG")))` were trout or sculpin tagged within the study area. `r nrow(filter(indiv_datasets_list$releasedata, Species %in% c("TGM")))` fish were Tiger Muskies tagged and released in Shadow Mountain Reservoir, and `r nrow(filter(indiv_datasets_list$releasedata, Species %in% c("MERG")))` tags were mergansers tagged as part of an avian predation study. There have been `r nrow(indiv_datasets_list$recapdata)` instances of fish recapture throughout the study period. The table below breaks down release and recapture data by year and species on tags other than Tiger Muskies. 
...
```{r fish Tagged by length, echo = FALSE, include=TRUE, warning=FALSE,message=FALSE,error=FALSE }
# number of fish tagged 
plot <- ReleaseData %>%
  rename(`Length (mm)` = Length) %>%
  ggplot(aes(x = `Length (mm)`, fill = Species) ) +
  geom_histogram(binwidth = 20)+
  theme_classic() +
  labs(title = "Released Fish by Length (mm)", caption = "Binwidth = 20mm") +
...
LWRanges <- ReleaseData %>%
  ...
  summarise(`Minimum Length (mm)` = round(min(Length, na.rm = T), 2), 
            ...
            `SD Length (mm)` = round(sd(Length, na.rm = T), 2),
            ...
            `SD Weight (g)` = round(sd(Weight, na.rm = T), 2))
...
```
There have been a total of `r length(unique(indiv_datasets_list$releasedata$ReleaseSite))` sites where tagged fish have been released and `r length(unique(indiv_datasets_list$recapdata$RecaptureSite))` sites where fish have been recaptured over the course of the study. The following tables show yearly counts of released and recaptured fish by site and species. 
...
datatable(ReleaseCounts, 
          caption = "Release Summaries by Site",
  options = list(
    pageLength = 10,
    autoWidth = TRUE
  ),
...
datatable(RecaptureCounts, 
          caption = "Recapture Summaries by Site",
  options = list(
    pageLength = 10,
    autoWidth = TRUE
  ),
...
Fish detected by antennas, released, or recaptured are assigned a one letter "State", defined as a specific section of the river outlined in the map below. 
The following tables summarize activity in the Connectivity Channel (CRCC) as defined by movement between the "States" upstream of the channel, downstream of the channel, and within the channel throughout a fish's encounter history. Antenna locations can be toggled with the layer control. 

```{r, include=TRUE, echo=FALSE}

leaflet() %>% #Warning: Error in UseMethod: no applicable method for 'metaData' applied to an object of class "NULL"  solved becuase leaflet() needs an arg leaflet(x)
  ...
  addAwesomeMarkers(data = antenna_sites,
                            ...
                            group = "Antennas") %>%
  addLayersControl(overlayGroups = c("Antennas", "States"), 
                   baseGroups = c("Satellite")
  ) %>%
  hideGroup(c("Antennas")) %>%
...
usedChannelSummarizedTotal <- usedChannel %>%
  summarise(`Total Fish Detected In Channel (Including Release)` = n())
...
movementsLengths <- movementsLength %>%
  count(`Length Bin (mm)`, movement_only) %>%
  arrange(`Length Bin (mm)`, movement_only) 

plot <- ggplot(movementsLengths, aes(x = `Length Bin (mm)`, y = n, fill = movement_only)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    x = "Length Bin (mm)",
...
datatable(movementsLengthsWide, 
          caption = "Movement Summary Table by Length Bins (mm)",
  options = list(
    pageLength = 10,
    autoWidth = TRUE
  ),
...
When a tagged fish swims across an antenna, it registers a detection on that antenna. When the fish remains on an antenna, it registers detections until the fish swims out of the detection field. Combined with realease and recapture data, there are `r nrow(combinedData_df_list$All_Detections)` rows of detection data to manage including marker tags and unknown tags. Of those, `r nrow(combinedData_df_list$All_Events)` are from known released fish. The following graph and table show the raw amount of detections per antenna across the entirety of the study. 
```{r, include=TRUE, echo=FALSE}
...
stationaryAntennaTimeSummaryMessage <- paste0("Stationary Antennas were down for ", totalDowntimePercentage, "% of the total time across all Stationary Antennas. All individual Stationary antennas were down for a total of ", round(stationaryAntennasDown$`Total Time Antennas were down`, 2), " days out of a total of ", round(StationaryAntennaTimeTotal$`Total Duration`, 2), " days of total time across all Stationary antennas.")
...
biomarkAntennaTimeSummaryMessage <- paste0("Biomark antennas were down for ", totalBiomarkDowntimePercentage, "% of the total time across all Biomark antennas. All individual Biomark antennas were down for a total of ", round(biomarkAntennasDown$`Total Time Antennas were down`, 2), " days out of a total of ", round(biomarkAntennaTimeTotal$`Total Duration`, 2), " days of total time across all Biomark Antennas.")
...
frequenciesSummarized <- All_EventsData %>%
            ...
            count(Event, Year = year(Date), name = "Raw Detections (Released Fish Only)") %>%
  arrange(desc(Year)) %>%
...
frequenciesSummarizedType <- All_EventsData %>%
  mutate(detType = case_when(Event %in% StationaryAntennasFrontends$FrontendSiteCode ~ "Stationary", 
                             ...
                             #want to inlcude ghostbuster in our mobiel analysis
                                 Event %in% c(mobileCodes$FrontendSiteCode, "G1") ~ "Mobile",
                             TRUE ~ Event
                                 ))

frequenciesSummarizedType1 <- frequenciesSummarizedType %>%
  count(`Event Type` = detType, Year = year(Date), name = "Raw Detections (Released Fish Only)") %>%
  arrange(desc(Year)) 
#made 
frequenciesSummarizedTypeOverall <- frequenciesSummarizedType %>%
  count(`Event Type` = detType, name = "Raw Detections (Released Fish Only)")

datatable(frequenciesSummarizedTypeOverall,
          rownames = FALSE,
          caption = c("Raw Number of Fish Detections by Detection Type"),
          extensions = c('Buttons'),
          #for slider filter instead of text input
          filter = 'top',
          options = list(
...
```
Sometimes these antennas lose power for a certain period of time. 
`r stationaryAntennaTimeSummaryMessage`
`r biomarkAntennaTimeSummaryMessage`

## Unique Detections

```{r uniqueDetections, include=TRUE, echo=FALSE}
...
# #weirdly excludes NA entries as well
# noTGMReleaseFile <- ReleaseData %>%
#   filter(Species != "TGM")

detectionsRecapsOnly <- frequenciesSummarizedType %>%
  filter(!detType %in% c("Release", "Recapture and Release", "G1"), 
         TAG %in% ReleaseData$TagID)

#ill have duplicate entries if a tag is captured in more than one method
detectionsNotUnique <- detectionsRecapsOnly %>%
  distinct(TAG, detType) %>%
  count(detType, name = "Released Tags Detected")
  #anti_join(ReleaseData)

#how many of the total fish in release file have been captured by any method?
unqiueFishfromReleaseFileCaptured <- detectionsRecapsOnly %>%
  distinct(TAG)
...
#populate with other table
uniquedetectionsTable <- detectionsNotUnique %>%
  mutate(`Unique Tags` = as.numeric(NA))

uniquedetectionsTable[uniquedetectionsTable[["detType"]] == "Stationary", "Unique Tags"] <- numberofStationaryUniqueTags
uniquedetectionsTable[uniquedetectionsTable[["detType"]] == "Biomark", "Unique Tags"] <- numberofBiomarkUniqueTags
uniquedetectionsTable[uniquedetectionsTable[["detType"]] == "Mobile", "Unique Tags"] <- numberofMobileUniqueTags
uniquedetectionsTable[uniquedetectionsTable[["detType"]] == "Recapture", "Unique Tags"] <- numberofRecappedUniqueTags

uniquedetectionsTableTotalDetects <- uniquedetectionsTable %>%
  left_join(frequenciesSummarizedTypeOverall, by = c("detType" = "Event Type"))

uniquedetectionsTableReformatted <- uniquedetectionsTableTotalDetects %>%
  mutate(`% Tags Detected` = `Released Tags Detected`/nrow(ReleaseData)) %>%
  rename(Method = detType, 
         `Total Overall Detections (Released Fish Only)` = `Raw Detections (Released Fish Only)`) %>%
  dplyr::relocate(`% Tags Detected`, .after = `Released Tags Detected`) %>%
  dplyr::relocate(`Total Overall Detections (Released Fish Only)`, .after = `Method`) 

uniqueTable <- datatable(uniquedetectionsTableReformatted,
          rownames = FALSE,
          caption = c("Detections by Method"),
          #extensions = c('Buttons'),
          #for slider filter instead of text input
          filter = 'top'
          # options = list(
...
#additionally, mobile data and recaps are not used for this method which is why these numbers vary between the "CRCC Usage" table in the section above
...
###MESSAGES
totalBeforeSequenceMessage <- paste0("Before the Connectivity Channel was built, there were ", nrow(beforeHPCFSequences), " upstream journeys from Hitching Post Antenna or Windy Gap Biomark Antenna (both located downstream of Windy Gap dam) to Confluence Antenna (located upstream of Windy Gap dam) by non-Avian Predated fish and ", nrow(beforeCFHPSequences), " downstream journeys from Confluence Antenna to Hitching Post Antenna or Windy Gap Biomark Antenna, over the course of ", round(totalDaysBefore, 2), " days. This equates to one upstream journey every ", round(as.numeric(totalDaysBefore)/nrow(beforeHPCFSequences), 2), " days and one downstream journey every ", round(as.numeric(totalDaysAfter)/nrow(beforeCFHPSequences), 2), " days.") 
...
## Avian Predation
Throughout the duration of the study, we have identified `r length(unique(indiv_datasets_list$avian_preddata$TagID))` fish that have been predated by birds. These tags are found a few different ways:

 *  By identifying missing antennas in their encounter history, ie hitting an antenna different than the ones we'd expect them to hit
 *  Identifying instances where a tag moved quickly, or disproportionately far throughout its encounter history or even just one move
 * Tagging actual birds known to predate fish (in this case, Mergansers) and comparing their encounter histories to suspected predated tags. 
...
```{r mergDataTable, echo=FALSE, include=TRUE,warning=FALSE,message=FALSE}
#function for formatting seconds
computeTimeDiffAnnualReport <- function(secondsValue) {
  if (is.na(secondsValue)) {
    return("First Detection in History") # Or "N/A", or "Missing Data"
  }
  diff_secs <- abs(as.numeric(secondsValue))
  if (diff_secs < 60) {
...
mergOnly <- combinedData_df_list$All_Events %>%
  filter(TAG == "230000087407")

mergOnly1 <- mergOnly %>%
  arrange(Datetime) %>%
    mutate(
           `Time Since Previous Detection (For Sorting)` = difftime(lag(Datetime), Datetime, units = "secs"), 
           `Time Since Previous Detection (User Friendly)` = mapply(computeTimeDiffAnnualReport, `Time Since Previous Detection (For Sorting)`), 
           `Previous Event` = lag(Event)
           ) %>%
  select(TAG, Datetime,  Species, Event, `Previous Event`,  `Time Since Previous Detection (For Sorting)`, `Time Since Previous Detection (User Friendly)`)

datatable(
    mergOnly1,
    caption = "Tagged Merganser Encounter History", 
    rownames = FALSE,
    selection = "single",
    options = list(
      stateSave = TRUE,
...
```{r mergAnimation, echo=FALSE, include=TRUE,warning=FALSE,message=FALSE}
...
AllEvents <- mergOnly %>%
  filter(Date >= as.Date("2024-09-17") & Date <= as.Date("2024-09-21")
    ) %>%
...
map_with_data <- ggplot() +
  basemap_gglayer(coords1, verbose = FALSE) +
  scale_fill_identity() +
  coord_sf() +
  theme_classic() +
...
map_with_data1 <- map_with_data + 
  ...
  map_with_data1 <- map_with_data1 +
    ...
    ggtitle(
      ...
      paste("Tagged Merganser:", '{frame_time}'), #{frame_time}
...
#map_with_data1
endPauseValue = 30
animate(map_with_data1, nframes = frameUnit + endPauseValue, end_pause = endPauseValue, fps = 10) #, height = 1200, width =1200
...
```{r troutDataTable, echo=FALSE, include=TRUE, warning=FALSE,message=FALSE}

fishOnly <- combinedData_df_list$All_Events %>%
...
datatable(
    fishOnly1,
    caption = "Tagged Brown Trout Encounter History", 
    rownames = FALSE,
    selection = "single",
    options = list(
      stateSave = TRUE,
...
```{r troutAnimation, echo=FALSE, include=TRUE,warning=FALSE,message=FALSE}
...
AllEvents <- fishOnly %>%
  filter(Date >= as.Date("2025-10-05") & Date <= as.Date("2025-10-08")
    ) %>%
  mutate(hours_since = as.numeric(floor(difftime(Datetime, min(Datetime), units = "hours"))), 
         hourSequence = as_datetime(as.character(round(Datetime, units = "hours"))), 
         days_since = as.numeric(ceiling(difftime(Date, min(Date), units = "days"))),
