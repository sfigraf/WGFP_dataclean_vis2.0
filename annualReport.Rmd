...
uniqueDetectionsMessage <- paste0("Stationary Antennas detected ", numberofStationaryUniqueTags, " fish that were not detected by mobile, biomark, or recapture methods.
Biomark Antennas detected ", numberofBiomarkUniqueTags, " fish that were not detected by mobile, stationary, or recapture methods.  
Mobile Antennas detected ", numberofMobileUniqueTags, " fish that were not detected by biomark, stationary, or recapture methods.  
There were ", numberofRecappedUniqueTags, " fish recaptured that were not detected by biomark, stationary, or recapture methods."      
                                  )
...
#easy way is to run get sequences on a bunch of different options
#trickier way with more up front loading but less later on is to use condensed states from createMarkHistories, see where a fish hit different states consecutively
...
beforeEndDate <- ymd_hms("2023-10-18 12:44:05") #first CRCC antenna online
...
# #get sequences with Hp, wg antenas to CF after
...
totalAfterSequences <- nrow(afterHPCFSequencesThroughChannel1) + nrow(afterHPCFSequencesThroughChannel) + nrow(afterHPCFSequencesThroughChannel2)
totalAfterSequenceMessage <- paste0("There have been ", totalAfterSequences, " journeys from Hitching Post Antenna or Windy Gap Biomark Antenna to Confluence Antenna by non-Avian Predated fish AFTER Channel was built, over the course of ", round(totalDaysAfter, 2), " days. This equates to 1 journey every ", round(as.numeric(totalDaysAfter)/totalAfterSequences, 2), " days.")

totalBeforeSequenceMessage <- paste0("There were ", nrow(beforeHPCFSequences), " journeys from Hitching Post Antenna or Windy Gap Biomark Antenna below the dam to Confluence Antenna by non-Avian Predated fish BEFORE Channel was built, over the course of ", round(totalDaysBefore, 2), " days. This equates to 1 journey every ", round(as.numeric(totalDaysBefore)/nrow(beforeHPCFSequences), 2), " days.") 

#make percentages or avergaes: aka "there was one sequence every 3 days before, nowe theres one sequence every 2 days" coomapred to RB/HP sequences

#compare with movements from Red Barn to Hitching Post 
#rb to hp before
rbHPBefore <- invisible(getSequences(beforeDataAvianPredationFiltered, c("RB"), "", c("HP"), c("M1", "M2")))

rbHPAfter <- invisible(getSequences(afterDataAvianPredationFiltered, c("RB"), "", c("HP"), c("M1", "M2")))

beforeRBtoHPSequencesMessage <- paste0("There were ", nrow(rbHPBefore), " journeys from Red Barn Antenna to Hitching Post Antenna by non-Avian predated fish BEFORE Channel was built, over the course of ", round(totalDaysBefore, 2), " days. This equates to 1 journey every ", round(as.numeric(totalDaysBefore)/nrow(rbHPBefore), 2), " days.")

afterRBtoHPSequencesMessage <- paste0("There were ", nrow(rbHPAfter), " journeys from Red Barn Antenna to Hitching Post Antenna by non-Avian predated fish AFTER Channel was built, over the course of ", round(totalDaysAfter, 2), " days. This equates to 1 journey every ", round(as.numeric(totalDaysAfter)/nrow(rbHPAfter), 2), " days.")
